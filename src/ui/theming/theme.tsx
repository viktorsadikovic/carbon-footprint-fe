...
export function ThemeProvider({
  ...
  useEffect(() => {
    ...
    if (savedTheme) {
      ...
      dispatch(
        setActiveTheme(
          prefersLight ? SupportedThemes.LIGHT : SupportedThemes.DARK
        )
      );
    }
  }, []);
  ...
  const appTheme = useMemo(
    () =>
      createTheme({
        themedPalette:
          activeTheme === SupportedThemes.DARK ? darkPalette : lightPalette,
      }),
    [activeTheme]
  );
  return (
    <MaterialThemeProvider theme={appTheme}>{children}</MaterialThemeProvider>
  );
}
